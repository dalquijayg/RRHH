...
async function initApp() {
    try {
        ...
        Swal.fire({
            ...
            text: 'Seleccione un departamento para comenzar.',
            timer: 2000,
...
async function loadDepartmentInfo(deptId) {
    try {
        ...
        if (result.length > 0) {
            ...
            selectedDeptName = departmentInfo.NombreDepartamento;
...
async function loadStatusData(filters = {}) {
    try {
        ...
        const params = [selectedDepartmentId];
...
function initEvents() {
    ...
    if (departmentSelector) {
        departmentSelector.addEventListener('change', function() {
            ...
            if (!this.value) {
                departmentInfoSection.style.display = 'none';
                disableControls();
                showInitialMessage();
            }
        });
    ...
    if (searchDepartmentBtn) {
        searchDepartmentBtn.addEventListener('click', async function() {
            const deptId = departmentSelector.value;
