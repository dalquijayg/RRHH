...
// Inicializar la aplicación (actualizada para carga automática)
async function initApp() {
    try {
        const loadingSwal = mostrarCargando('Inicializando sistema...');
        ...
        // Obtener el departamento del usuario automáticamente
        await loadUserDepartmentInfo();
        
        // Cargar los colaboradores del departamento automáticamente
        await loadEmployees(userDepartmentId);
...
function loadUserData() {
    try {
        ...
        // Obtener el IdSucuDepa del usuario
        userDepartmentId = userData.IdSucuDepa;
        
        if (!userDepartmentId) {
            Swal.fire({
                icon: 'error',
                title: 'Error de configuración',
                text: 'Su usuario no tiene un departamento asignado. Contacte al administrador.',
                confirmButtonColor: '#FF9800'
            });
            return false;
        }
        
        console.log('Usuario cargado:', userData.NombreCompleto, 'Departamento ID:', userDepartmentId);
        return true;
...
// Cargar información del departamento del usuario
async function loadUserDepartmentInfo() {
    try {
        ...
        const result = await connection.query(query, [userDepartmentId]);
        await connection.close();
        
        if (result.length > 0) {
            const departmentInfo = result[0];
            userDepartmentName = departmentInfo.NombreDepartamento;
            ...
            Swal.fire({
                ...
                text: 'No se pudo encontrar información de su departamento.',
                confirmButtonColor: '#FF9800'
            });
        }
    } catch (error) {
        console.error('Error al cargar información del departamento:', error);
        userDepartmentName = 'Error al cargar';
        document.getElementById('departmentName').textContent = userDepartmentName;
        throw error;
    }
}
...
async function loadEmployees(deptId) {
    try {
        ...
        if (result.length > 0) {
            ...
            console.log(`Cargados ${employeesData.length} colaboradores del departamento ${userDepartmentName}`);
                ...
                <tr>
                    <td colspan="7" style="text-align: center; padding: 20px;">
                        <div class="empty-state">
                            <i class="fas fa-users" style="font-size: 56px; color: #ddd; margin-bottom: 20px; display: block;"></i>
                            <h3 style="font-size: 22px; color: #666; margin-bottom: 10px;">No hay colaboradores</h3>
                            <p style="font-size: 15px; color: #999;">No se encontraron colaboradores activos en su departamento.</p>
                        </div>
                    </td>
                </tr>
            `;
...
async function loadSpecialDays() {
    try {
        ...
        // Usar el departamento del usuario logueado
        const result = await connection.query(query, [userDepartmentId]);
        await connection.close();
        ...
        if (result.length > 0) {
            ...
            console.log(`Cargados ${specialDays.length} días especiales para el departamento`);
...
function loadHolyWeekDates() {
    ...
    console.log(`Cargadas ${holyWeekDates.length} fechas de Semana Santa`);
}
...
function showWelcomeMessage() {
    ...
    Toast.fire({
        icon: 'success',
        title: `¡Bienvenido/a ${userData.NombreCompleto}!`,
        text: `Departamento: ${userDepartmentName}`
    });
}
...
function renderEmployeesTable() {
            ...
            <tr>
                <td colspan="7" style="text-align: center; padding: 20px;">
                    <div class="empty-state">
                        ...
                        <h3 style="font-size: 18px; color: #666; margin-bottom: 8px;">No se encontraron colaboradores</h3>
                        <p style="font-size: 14px; color: #999;">No hay colaboradores que coincidan con su búsqueda en <strong>${userDepartmentName}</strong>.</p>
                    </div>
                </td>
            </tr>
        `;
    ...
    currentPageData.forEach((employee, index) => {
        ...
        if (diasVacaciones <= 0) {
            ...
            diasDisponiblesStyle = `<div class="days-count" style="background-color: #FF9800;">${diasVacaciones} días</div>`;
        } else if (diasVacaciones < 60) {
            diasDisponiblesStyle = `<div class="days-count" style="background-color: #FFC107;">${diasVacaciones} días</div>`;
        } else {
        ...
        row.innerHTML = `
            ...
            <td>${hireDate}</td>
            <td>${employee.AniosCumplidos || 0} años</td>
...
async function refreshData() {
    try {
        ...
        // Recargar información del departamento
        await loadUserDepartmentInfo();
        
        // Recargar empleados
        await loadEmployees(userDepartmentId);
        
        // Recargar días especiales
        await loadSpecialDays();
        ...
        // Remover clase de loading del botón
        const refreshBtn = document.getElementById('refreshBtn');
        if (refreshBtn) {
...
// Función para exportar a Excel (actualizada para el nuevo sistema)
async function exportToExcel() {
    if (!userDepartmentId || !userDepartmentName) {
        Swal.fire({
            icon: 'warning',
            title: 'Error de datos',
            text: 'No se pudo obtener información del departamento para exportar.',
            confirmButtonColor: '#FF9800'
        });
        return;
    }
    ...
    try {
        const loadingSwal = mostrarCargando('Generando reporte Excel...');
        ...
        // Agregar datos de empleados (usar filteredData para respetar búsquedas)
        const dataToExport = filteredData.length > 0 ? filteredData : employeesData;
        ...
        dataToExport.forEach((employee, index) => {
            ...
            const row = worksheet.addRow({
                ...
                aniosServicio: `${employee.AniosCumplidos || 0} años`,
                diasDisponibles: `${employee.DiasVacaciones || 0} días`,
        ...
        worksheet.spliceRows(1, 0, [`REPORTE DE VACACIONES - ${userDepartmentName.toUpperCase()}`]);
        worksheet.spliceRows(2, 0, [`Fecha de generación: ${new Date().toLocaleDateString('es-GT')} ${new Date().toLocaleTimeString('es-GT')}`]);
        worksheet.spliceRows(3, 0, [`Generado por: ${userData.NombreCompleto}`]);
        ...
        userCell.font = { italic: true, size: 11, color: { argb: 'FF777777' } };
        ...
        const fileName = `Vacaciones_${userDepartmentName.replace(/\s+/g, '_')}_${formattedDate}.xlsx`;
        ...
        Swal.fire({
            ...
            text: 'No se pudo generar el reporte Excel. Intente nuevamente.',
            confirmButtonColor: '#FF9800'
        });
    }
}
    ...
    try {
        ...
        const result = await Swal.fire({
            ...
            html: `
                <div style="text-align: left; margin: 20px 0;">
                    ...
                    <p><strong>Departamento:</strong> ${userDepartmentName}</p>
        ...
        if (result.isConfirmed) {
            ...
            try {
                ...
                // Actualizar los datos de empleados para reflejar los cambios
                await loadEmployees(userDepartmentId);
...
async function generarPDFPeriodoCompleto(empleado, periodo) {
    try {
        ...
        doc.setFont(undefined, 'normal');
        doc.text(userDepartmentName, 70, y);
        y += 8;
...
async function obtenerHistorialVacaciones(employeeId, fechaInicio = null, fechaFin = null) {
    try {
        ...
        if (fechaInicio && fechaFin) {
            ...
            query += ` AND DATE(FechahoraRegistro) BETWEEN ? AND DATE(?)`;
            params.push(fechaInicio, fechaFinCompleta);
        }
        
        // Ordenar por fecha de registro más reciente primero
        query += ` ORDER BY FechahoraRegistro DESC`;
    ...
    if (resultados.length === 0) {
        resultsContainer.innerHTML = `
            <div class="no-modal-results">
                ...
                <p>No se encontraron colaboradores en ${userDepartmentName} que coincidan con su búsqueda</p>
            </div>
        `;
        return;
    }
            ...
            <div class="modal-search-item" data-id="${employee.IdPersonal}">
                ...
                <div class="search-result-info">
                    ...
                    <div class="search-result-days">
                        <i class="fas fa-calendar-day"></i> ${employee.DiasVacaciones || 0} días disponibles
                    </div>
                </div>
            </div>
        `;
...
// Obtener períodos completados para el generador de PDF
...
// Inicializar eventos del modal PDF
function initializePDFSearchEvents() {
    const searchInput = document.getElementById('pdfEmployeeSearch');
...
// Buscar empleados para PDF
function searchEmployeesForPDF(query) {
    ...
    // Filtrar empleados del departamento actual (excluyendo al usuario actual)
    const searchLower = query.toLowerCase();
    ...
    // Mostrar resultados
    displaySearchResultsForPDF(currentSearchResults, query);
}

// Mostrar resultados de búsqueda para PDF
function displaySearchResultsForPDF(results, query) {
    const resultsContainer = document.getElementById('searchResults');
    ...
    if (results.length === 0) {
        resultsContainer.innerHTML = `
            <div class="no-results">
                <i class="fas fa-search"></i>
                <p>No se encontraron colaboradores en ${userDepartmentName}</p>
            </div>
        `;
        return;
    }
    ...
    document.querySelectorAll('.search-result-item').forEach(item => {
        item.addEventListener('click', function() {
            ...
            selectEmployeeForPDF(itemEmployeeId);
        });
    });
}

// Seleccionar empleado para PDF
function selectEmployeeForPDF(selectedEmployeeId) {
    ...
    handleEmployeeSelectionForPDF(selectedEmployeeId);
}

// Manejar selección de empleado para PDF
async function handleEmployeeSelectionForPDF(employeeId) {
    const periodSelect = document.getElementById('pdfPeriodSelect');
    ...
    const loadingSwal = Swal.fire({
        ...
        html: `
            <div class="loading-animation">
                <div class="spinner"></div>
                <p>Buscando períodos completados para ${userDepartmentName}</p>
            </div>
        `,
        showConfirmButton: false,
        allowOutsideClick: false,
    ...
    try {
        ...
        if (periodosCompletados.length === 0) {
            ...
            Swal.fire({
                ...
                html: `
                    <p><strong>${getFullName(employee)}</strong> no tiene períodos de 15 días completados en ${userDepartmentName}.</p>
    ...
    try {
        ...
        try {
            ...
            Swal.fire({
                icon: 'success',
                title: 'PDF Generado',
                text: `La ficha de control de ${getFullName(employee)} se ha generado correctamente.`,
                confirmButtonColor: '#4CAF50',
                timer: 3000,
                timerProgressBar: true
            });
...
// Inicializar eventos (actualizada para el nuevo sistema automático)
function initEvents() {
    // Búsqueda
    if (searchInput) {
    ...
    // Eventos del modal de información
    ...
    if (searchEmployeeBtn) {
        searchEmployeeBtn.addEventListener('click', mostrarBuscadorEmpleados);
    }
